...
const checkConsent = (consentType, cookieString = document.cookie) => {
    switch (consentType) {
        case exports.ConsentType.Functional:
            return getFunctionalConsent(cookieString);
        case exports.ConsentType.Marketing:
            return getMarketingConsent(cookieString);
        case exports.ConsentType.Statistical:
            return getStatisticalConsent(cookieString);
        case exports.ConsentType.Essential:
            return true;
        default:
            return false;
    }
 ...
 */
function setItem$1(key, data, category, consentType = exports.ConsentType.Essential, cookieString = document.cookie) {
...
function replacePlaceholders(str, data) {
    ...
    return ((str &&
        string &&
        str.replace(string, function (a, b) {
            return data[b];
        })) ||
        "");
}
...
function deepGet(path, originalScope) {
    if ({}.toString.call(originalScope) !== "[object Object]" ||
        {}.toString.call(path) !== "[object Array]" ||
        !path.length) {
        return null; // early exit (prevent unnecessary TypeErrors thrown to the user)
    }
    return path.reduce((acc, key, i) => acc && acc[key]
        ? acc[key] // match is found
        : acc[key] === 0 || acc[key] === false || acc[key] === ""
            ? acc[key] // 0, false and "" are valid values
            : // no match is found
                path[i + 1]
                    ? {} // not the end of the path (we cannot early exit 'reduce')
                    : null, // the end of the path
    originalScope);
}
/**
 * @public
...
function deepCompareObjectProperties(targetObject, referenceObject) {
    ...
    if (JSON.stringify(targetObjectKeys) !== JSON.stringify(referenceObjectKeys)) {
        ...
        targetObjectKeys.forEach((key) => typeof targetObject[key] === "object" &&
            typeof referenceObject[key] === "object" &&
            key !== "tasks" &&
            deepCompareObjectProperties(targetObject[key], referenceObject[key]));
    }
    return true;
}
/**
...
function flatten(data, prefix = "") {
    ...
    const nestElement = (prev, value, key) => {
        if (value && typeof value === "object") {
            ...
            return Object.assign(Object.assign({}, prev), { [`${prefix}${key}`]: value });
        }
    };
    ...
    if (!feature) {
        ...
        return customerRoles.length > 0
            ? feature
                ...
                if ((decisionRule === 4 && specialRoles && specialRoles.switchedConsignee) ||
                    (decisionRule === 3 && specialRoles && specialRoles.switchedShipper) ||
                    (Array.isArray(decisionRule) &&
                        decisionRule[0] === 4 &&
                        specialRoles &&
                        specialRoles.switchedConsignee) ||
                    (Array.isArray(decisionRule) && decisionRule[0] === 3 && specialRoles && specialRoles.switchedShipper)) {
                    return false;
                }
...
function sealandMaerskRegionSelected() {
    ...
    const regionCookieValue = (getCookies &&
        getCookies() &&
        getCookies().regionSelected);
...
const globalBaseConfig = {
    ...
    BRANDS: [
        ...
        },
        {
            ...
            designCode: "anrm",
            langCode: "anrm",
            commercialName: "Aliança - A Maersk company",
            domain: "alianca.com.br"
        }
    ],
    AO: {
        brands: {
    ...
    USER_ROLES: new Map([
        ...
        [6, "Additional Notify Party"],
        [7, "Allocation Owner"],
        [11, "Outward Customs Broker"],
        [12, "Inward Customs Broker"],
        [13, "Contractual Carrier's Inward Agent"],
        [14, "Invoice Party"],
        [15, "Outward Forwarder"],
        [16, "Inward Forwarder"],
        [22, "Transport Document Receiver"],
        [25, "Contractual Carrier's Outward Agent"],
        [26, "Credit Party"],
        [27, "Product Owner"],
        [28, "Outward Document Owner"],
        [29, "Inward Document Owner"],
        [31, "Release to Party"],
        [32, "Lawful (B/L) Holder"],
        [33, "Demurrage Invoice Party"],
        [34, "Detention Invoice Party"],
        [35, "Supplier"],
        [36, "House Shipper"],
        [37, "House Consignee"],
        [38, "Switched Shipper"],
        [39, "Switched Consignee"],
        [40, "Doc Shipper"],
        [41, "Doc Consignee"],
        [44, "Price Owner"],
        [45, "Named Account Customer"],
        [46, "Export Deal Owner"],
        [47, "Import Deal Owner"],
        [48, "Import Demurrage Payer"],
        [49, "Import Detention Payer"]
    ])
};
...
function getBrandFromHostname(url, localOverride = "maersk") {
    ...
    return (globalBaseConfig.BRANDS.find((brand) => brand.brandName === domainToMatch) ||
        globalBaseConfig.BRANDS.find((brand) => brand.brandName === "maersk"));
}
/**
 * @public
...
function getEnvironmentIdentifier(url) {
    ...
    // Demo labels
    if ([/\.demo\./, /\.demo/, /demo\./, /\.stage\./, /\.stage/, /stage\./, /staging/].some((regex) => url === null || url === void 0 ? void 0 : url.hostname.match(regex))) {
        label = "demo";
    }
    // Preprod labels
 ...
 */
function setFaviconByBrand({ brandName }) {
    if (brandName) {
...
function attachElementToHead({ elemType, rel, link, innerText, media, property, content, name }) {
...
function insertIGNHeaderAndFooter({ brandCode, assetsLocationMap = {}, settings = {} }) {
...
function decodeJwtBody(jwt) {
    ...
    try {
        ...
        const utf8 = decodeURIComponent(binary
            .split("")
 ...
 */
function setItem(key, data, category, consentType = exports.ConsentType.Essential, cookieString = document.cookie) {
...
function convertPOSTtoGET({ url, fetchOptions, getUrlPath, correlationId }) {
    // this is POST to GET, not GET to anything else
    if (fetchOptions.method && !["PUT", "POST", "PATCH"].includes(fetchOptions.method)) {
        return { updatedUrl: url, updatedFetchOptions: fetchOptions };
    }
    if (getUrlPath) {
    ...
    return { updatedUrl: urlObject.href, updatedFetchOptions: fetchOptions };
}
/**
 * @public
 ...
 */
async function getRetryableResponse({ url, fetchOptions, retries = 5, timeout = 3000, getUrlPath }) {
    if (retries === 0) {
        return;
    }
    ...
    if (Array.isArray(json)) {
        if (json.length === 1)
            ...
            throw new Error("Response is a collection with zero or more then one element. It cannot be handled by multiple retryable requests.");
    }
    let updatedUrl = url;
...
function formatCurrency(amount, currency) {
    const lang = getSelectedLocale(globalBaseConfig.DEFAULT_LANGUAGE);
    const func = new Intl.NumberFormat(`${lang}-u-nu-latn`, // Use the language set in the cookie but in latin character set
    {
        style: "currency",
        currency: currency,
...
function maskData(numberOrString, config) {
    ...
    if (!config ||
        ...
        !(config.fromStart || config.fromEnd) ||
        !(typeof config.fromStart === "number" || typeof config.fromEnd === "number")) {
    ...
    if ((config.fromStart && convertedInput.length < config.fromStart) ||
        (config.fromEnd && convertedInput.length < config.fromEnd)) {
        throw new Error("The number of chars need not to be masked can not be greater than input");
    }
    let noOfcharsNotToBeMasked = 0, charsNotToBeMasked = "";
    if (config.fromStart) {
...
function maskCustomerCode(code) {
    return maskData(code, { fromEnd: 3 });
}
...
async function executeRequest({ url, method = "GET", body = null, headers = new Headers(), credentials = "same-origin", authorizedRequest = true, consumerKey = undefined, rawResponse = false, signal = undefined }) {
    if (!headers.has("Accept")) {
        headers.set("Accept", DEFAULT_CONTENT_TYPE);
    }
    if (body &&
        !headers.has("Content-Type") &&
        ["PUT", "POST", "PATCH", "DELETE"].includes(method.toUpperCase()) &&
    ...
    try {
        if (rawResponse) {
            ...
            if (response.headers.get("Content-Type") &&
                response.headers.get("Content-Type").indexOf(DEFAULT_CONTENT_TYPE) === 0) {
...
exports.apiDomain = apiDomain;
exports.bindEvent = bindEvent;
exports.canAccess = canAccess;
exports.checkAndSumProps = checkAndSumProps;
exports.checkConsent = checkConsent;
exports.decodeJwtBody = decodeJwtBody;
exports.decodeURIMessage = decodeURIMessage;
exports.deepCompareObjectProperties = deepCompareObjectProperties;
...
exports.getBetaFromHostname = getBetaFromHostname;
exports.getBrandFromHostname = getBrandFromHostname;
exports.getCookies = getCookies;
exports.getCustomerCode = getCustomerCode;
exports.getCustomerCodeFromJwt = getCustomerCodeFromJwt;
exports.getEnvironmentIdentifier = getEnvironmentIdentifier;
exports.getEnvironmentLabel = getEnvironmentLabel;
exports.getFunctionalConsent = getFunctionalConsent;
exports.getHeaderAndFooter = getHeaderAndFooter;
exports.getItem = getItem$1;
exports.getJwt = getJwt;
exports.getMarketingConsent = getMarketingConsent;
exports.getRetryableResponse = getRetryableResponse;
exports.getSalesForceAccessToken = getSalesForceAccessToken;
exports.getSelectedLangAttribute = getSelectedLangAttribute;
exports.getSelectedLanguage = getSelectedLanguage;
exports.getSelectedLocale = getSelectedLocale;
exports.getStatisticalConsent = getStatisticalConsent;
exports.getUrlParams = getUrlParams;
exports.getUserCredential = getUserCredential;
exports.getUsiJwt = getUsiJwt;
exports.globalBaseConfig = globalBaseConfig;
exports.goToOriginalUrl = goToOriginalUrl;
exports.isChinaDomain = isChinaDomain;
exports.isCustomerLoggedIn = isCustomerLoggedIn;
exports.isEmpty = isEmpty;
exports.isIE = isIE;
exports.isLocalhost = isLocalhost;
exports.isMobilePlatform = isMobilePlatform;
exports.isOverMax = isOverMax;
exports.isTwillSmeJourney = isTwillSmeJourney;
exports.kgsToLbs = kgsToLbs;
exports.langAttrMap = langAttrMap;
exports.languageMap = languageMap;
exports.lbsToKgs = lbsToKgs;
exports.localStore = localStore;
exports.makeKey = makeKey;
exports.maskCustomerCode = maskCustomerCode;
exports.maskData = maskData;
exports.matchesStoredNonce = matchesStoredNonce;
exports.redirectToLogin = redirectToLogin;
exports.redirectToSSP = redirectToSSP;
exports.removeItem = removeItem$1;
exports.removeRounding = removeRounding;
exports.replacePlaceholders = replacePlaceholders;
exports.sendOriginWindowOpener = sendOriginWindowOpener;
exports.sendOriginWindowParent = sendOriginWindowParent;
exports.sessionStore = sessionStore;
exports.setFaviconByBrand = setFaviconByBrand;
exports.setItem = setItem$1;
exports.setMaxCount = setMaxCount;
