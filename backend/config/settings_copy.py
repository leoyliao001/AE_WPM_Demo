...
ALLOWED_HOSTS = [
    ...
    for h in os.environ.get(
        ...
        "localhost,127.0.0.1,SCRBAEXDEFRM217,SCRBAEXDEFRM217.crb.apmoller.net,wpmpulse.crb.apmoller.net,wpmworkflow.crb.apmoller.net,crb.apmoller.net,10.176.111.18,*",
    ).split(",")
    if h.strip()
]
...
#
# Default: local SQLite (backend/db.sqlite3).
# SQL Server: set DJANGO_DB_ENGINE=mssql (uses Windows Trusted Connection unless DJANGO_DB_USER is set).
# Overrides: DJANGO_DB_NAME, DJANGO_DB_HOST, DJANGO_DB_PORT, DJANGO_DB_DRIVER, DJANGO_DB_EXTRA_PARAMS
...
if _DB_ENGINE in ("mssql", "sqlserver"):
    ...
    _db_config = {
        "ENGINE": "mssql",
        "NAME": os.environ.get("DJANGO_DB_NAME", "WPM"),
        "HOST": os.environ.get("DJANGO_DB_HOST", "SCRBAEXDEFRM259"),
...
CORS_ALLOWED_ORIGINS = [
    ...
    for origin in os.environ.get(
        "DJANGO_CORS_ORIGINS",
        "http://localhost:3001,http://127.0.0.1:3001,http://localhost,http://127.0.0.1,httpss://10.176.111.18,http://SCRBAEXDEFRM217,https://wpmpulse.crb.apmoller.net,https://wpmworkflow.crb.apmoller.net,https://crb.apmoller.net",
    ).split(",")
    if origin.strip()
]
