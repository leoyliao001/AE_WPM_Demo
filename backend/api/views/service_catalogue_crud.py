"""
Service Catalogue CRUD — create / update / delete for Handsontable edits.

Endpoints:
  POST   /api/service-catalogue/data/         body: { "uniqueData": [ {...}, ... ] }
  DELETE /api/service-catalogue/data/delete/  body: { "removedIds": [1, 2, ...] }
"""
...
from api.views.service_catalogue import ALL_FIELDS
...
def save_service_catalogue(request):
    ...
    for index, item in enumerate(unique_data):
        ...
        try:
            if _is_new_record(record_id):
                obj = ServiceCatalogue.objects.create(**payload)
                created_ids.append(obj.id)
            else:
                obj = ServiceCatalogue.objects.get(pk=int(record_id))
...
@api_view(["DELETE"])
@require_attributes_access("service_catalogue")
def delete_service_catalogue(request):
    ...
    for record_id in removed_ids:
        try:
            pk = int(record_id)
            deleted, _ = ServiceCatalogue.objects.filter(pk=pk).delete()
            if deleted:
