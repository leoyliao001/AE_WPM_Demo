"""
Working Hours CRUD — create / update / delete for Handsontable edits.

Endpoints:
  POST   /api/working-hours/data/         body: { "uniqueData": [ {...}, ... ] }
  DELETE /api/working-hours/data/delete/  body: { "removedIds": [1, 2, ...] }
"""
...
from api.views.working_hours import ALL_FIELDS
    ...
    for index, item in enumerate(unique_data):
        ...
        try:
            if _is_new_record(record_id):
                obj = WorkingHours.objects.create(**payload)
                created_ids.append(obj.id)
            else:
                obj = WorkingHours.objects.get(pk=int(record_id))
...
@api_view(["DELETE"])
@require_attributes_access("working_hours")
def delete_working_hours(request):
    ...
    for record_id in removed_ids:
        try:
            pk = int(record_id)
            deleted, _ = WorkingHours.objects.filter(pk=pk).delete()
            if deleted:
