...
from api.views.bpm_actual import ALL_FIELDS
...
def _derive_year(onboarding_month) -> int:
    ...
    match = re.search(r"(19|20)\d{2}", month_text)
    if match:
    ...
    for index, item in enumerate(unique_data):
        ...
        try:
            if _is_new_record(record_id):
                obj = BpmActual.objects.create(**payload)
                created_ids.append(obj.id)
            else:
                obj = BpmActual.objects.get(pk=int(record_id))
                for field, value in payload.items():
                    setattr(obj, field, value)
                obj.save(update_fields=[field for field in WRITABLE_FIELDS if field != "id"]) 
                updated_ids.append(obj.id)
        except BpmActual.DoesNotExist:
...
def delete_bpm_actual(request):
    ...
    for record_id in removed_ids:
        try:
            pk = int(record_id)
            deleted, _ = BpmActual.objects.filter(pk=pk).delete()
            if deleted:
